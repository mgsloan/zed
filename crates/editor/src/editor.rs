...
impl Editor {
    ...
    // agent: Avoid re-querying the completions and if `is_incomplete = false` and the query is a suffix of the query.
    //
    // The completions menu (one of the cases of self.context_menu) stores the current set of
        ...
        self.transact(window, cx, |this, window, cx| {
            ...
            for selection in &mut selections {
                if selection.is_empty() {
                    ...
                    // todo! safe change?
                    selection.set_tail(new_head, SelectionGoal::None);
                }
            }
