...
impl InspectorElementId {
    fn reversed_entries(
        ...
        &Option<ElementId>,
        &Option<(&'static std::panic::Location<'static>, usize)>,
    )> {
        let mut current = &self.0;
        let mut reversed_entries = Vec::new();
        loop {
            reversed_entries.push((&current.element_id, &current.location_and_instance_id));
...
impl Eq for InspectorElementId {}

impl PartialEq for InspectorElementId {
    fn eq(&self, other: &Self) -> bool {
        // Theoretically this could produce false positives, but in practice it's pretty much
        // impossible. Comparing the hashes alone has extremely rare false positives.
        &self.0.element_path_hash == &other.0.element_path_hash
            && &self.0.location_and_instance_id == &other.0.location_and_instance_id
            && &self.0.element_id == &other.0.element_id
    }
}

impl Hash for InspectorElementId {
    fn hash<H: Hasher>(&self, state: &mut H) {
        state.write_u64(self.0.element_path_hash);
        self.0.location_and_instance_id.hash(state);
    }
}

impl Debug for InspectorElementId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_list()
            .entries(self.reversed_entries().into_iter().rev())
            .finish()
    }
}
...
impl Display for InspectorElementId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for (i, entry) in self.reversed_entries().into_iter().rev().enumerate() {
            if i > 0 {
                write!(f, ".")?;
            }
            if let Some(element_id) = entry.0 {
                write!(f, "{}", element_id)?;
            }
            if let Some((location, instance_id)) = entry.1 {
                write!(f, "({} {})", location, instance_id)?;
            }
        }
        Ok(())
    }
    ...
    pub fn render_inspector_states(
        ...
        if let Some(active_element) = self.active_element.take() {
            for (type_id, state) in &active_element.states {
                ...
                {
                    let mut element =
                        (render_inspector)(active_element.id.clone(), state.as_ref(), window, cx);
