...
use crate::{
    identifier_references::{ReferenceRegion, local_identifiers},
    zed_code::Language,
};
...
impl IdentifierOccurrences {
    ...
    #[allow(dead_code)]
    pub fn within_treesitter_identifiers(
        language: &Language,
        tree: &tree_sitter::Tree,
        source: &str,
        offset_range: Range<usize>,
    ) -> Self {
        // TODO: Use `within_string` on comments / string literals? At that point there may not be
        // much benefit to using AST. Score these matches lower?
        ...
        Self::from_iterator(
            references
                .iter()
                .map(|reference| reference.identifier.0.as_ref()),
        )
    }
