...
async function handleBuffer(prettier) {
  for await (const messageText of readStdin()) {
    ...
    try {
      ...
      sendResponse(makeError("Parse error in message sent to prettier", `${e}\nmessage to prettier: "${messageText}"`));
      continue;
    }
    ...
    handleMessage(message, prettier).catch((e) => {
      ...
      sendResponse({
        id: message.id,
        ...makeError(`${e}`, `While handling prettier request: ${JSON.stringify(message)}`),
      });
    });
  }
}
